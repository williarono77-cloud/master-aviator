...
export default function BetPanel({ panelId = '1', side = 'top', session, onBetClick, disabled = false }) {
  ...
  return (
    <div className="bet-panel" data-side={side}>
      <div className="bet-panel__header">{side === 'top' ? 'Top (≥1.0x)' : 'Bottom (<1.0x)'}</div>
      ...
      <button type="button" className="bet-panel__bet-btn" onClick={handleBet} disabled={disabled} aria-disabled={disabled}>
        <span>Bet</span>
