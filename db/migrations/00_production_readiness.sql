...
-- -----------------------------------------------------------------------------
-- DB2.7 get_next_rounds_public RPC (frontend uses this)
-- -----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.get_next_rounds_public()
RETURNS TABLE(id uuid, round_id text, round_number int, status text, created_at timestamptz)
LANGUAGE sql
SECURITY DEFINER
SET search_path = public
AS $$
  SELECT r.id, r.round_id, r.round_number, r.status, r.created_at
